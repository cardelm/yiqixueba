...
function display(id) {
	jq(id).style.display = jq(id).style.display == '' ? 'none' : '';
}
...
function trim(str) {
	return (str + '').replace(/(\s+)jq/g, '').replace(/^\s+/g, '');
}
...
function thumbImg(obj, method) {
	...
	if(zw < 2) {
		...
		setTimeout("thumbImg(jq('" + obj.id + "'), " + method + ")", 100);
		return;
	}
	zr = zw / zh;
	method = !method ? 0 : 1;
...
function saveData(ignoreempty) {
	...
	var obj = jq('postform') && ((jq('fwin_newthread') && jq('fwin_newthread').style.display == '') || (jq('fwin_reply') && jq('fwin_reply').style.display == '')) ? jq('postform') : (jq('fastpostform') ? jq('fastpostform') : jq('postform'));
	if(!obj) return;
...
function switchAdvanceMode(url) {
	var obj = jq('postform') && ((jq('fwin_newthread') && jq('fwin_newthread').style.display == '') || (jq('fwin_reply') && jq('fwin_reply').style.display == '')) ? jq('postform') : jq('fastpostform');
...
function sidebar_collapse(lang) {
	if(lang[0]) {
		...
		jq('wrap').className = jq('wrap').className == 'wrap with_side s_clear' ? 'wrap s_clear' : 'wrap with_side s_clear';
	} else {
		var collapsed = getcookie('discuz_collapse');
...
function showMenu(v) {
	...
	var ctrlObj = jq(ctrlid);
	var menuObj = jq(menuid);
	if(!menuObj) return;
	...
	var dragobj = drag && jq(drag) ? jq(drag) : menuObj;
	...
	if(!menuObj.initialized) {
		...
		if(cover) {
			...
			jq('append_parent').appendChild(coverObj);
		}
	}
	menuObj.style.display = '';
	if(cover) jq(menuid + '_cover').style.display = '';
	if(fade) {
		var O = 0;
...
function setMenuPosition(showid, menuid, pos) {
	var showObj = jq(showid);
	var menuObj = menuid ? jq(menuid) : jq(showid + '_menu');
	...
	if(attr == '') {
		...
		var hide = function() {
			if(menuObj.cache) {
				...
				if(menuObj.cover) jq(attr + '_cover').style.display = 'none';
			} else {
				menuObj.parentNode.removeChild(menuObj);
				if(menuObj.cover) jq(attr + '_cover').parentNode.removeChild(jq(attr + '_cover'));
			}
			var tmp = [];
...
function showPrompt(ctrlid, evt, msg, timeout, negligible) {
	...
	if(!jq(menuid)) {
		...
		jq('append_parent').appendChild(div);
		if(ctrlid) {
	...
	if(ctrlid) {
		if(jq(ctrlid).evt !== false) {
			var prompting = function() {
				showMenu({'mtype':'prompt','ctrlid':ctrlid,'evt':evt,'menuid':menuid,'pos':'210'});
			};
			if(evt == 'click') {
				jq(ctrlid).onclick = prompting;
			} else {
				jq(ctrlid).onmouseover = prompting;
			}
		}
...
function showDialog(msg, mode, t, func, cover) {
	...
	menuObj.id = menuid;
	jq('append_parent').appendChild(menuObj);
	...
	menuObj.innerHTML = s;
	if(jq('fwin_dialog_submit')) jq('fwin_dialog_submit').onclick = function() {
...
function showWindow(k, url, mode, cache) {
	...
	if(!menuObj) {
		...
		fetchContent();
	} else if((mode == 'get' && url != menuObj.url) || (mode == 'post' && jq(url).action != menuObj.act)) {
		fetchContent();
	} else {
		show();
	}
...
function appendscript(src, text, reload, charset) {
	...
	if(reload && jq(id)) {
		jq(id).parentNode.removeChild(jq(id));
	}
	...
	try {
		...
		jq('append_parent').appendChild(scriptNode);
	} catch(e) {}
}
	...
	if(typeof o == 'object' && o.getAttribute) {
		if(o.getAttribute('ajaxtarget')) {
			...
			if(o.getAttribute('ajaxfunc')) {
				...
				_attachEvent(o, ajaxevent, newfunction(RegExp.jq1, RegExp.jq2));
			}
		}
	}
}
...
function ajaxpost(formid, showid, waitid, showidclass, submitbtn, recall) {
	...
	var ajaxframe = jq(ajaxframeid);
	var formtarget = jq(formid).target;
	...
	var handleResult = function() {
		...
		try {
			if(BROWSER.ie) {
				s = jq(ajaxframeid).contentWindow.document.XMLDocument.text;
			} else {
				s = jq(ajaxframeid).contentWindow.document.documentElement.firstChild.nodeValue;
			}
		} catch(e) {
			if(AJAX['debug']) {
				var error = mb_cutstr(jq(ajaxframeid).contentWindow.document.body.innerText.replace(/\r?\n/g, '\\n').replace(/"/g, '\\\"'), 200);
		...
		if(showidclass) {
			jq(showid).className = showidclass;
			if(submitbtn) {
				submitbtn.disabled = false;
			}
		}
		if(!evaled && (typeof ajaxerror == 'undefined' || !ajaxerror)) {
			ajaxinnerhtml(jq(showid), s);
		}
		ajaxerror = null;
		if(jq(formid)) jq(formid).target = formtarget;
		if(typeof recall == 'function') {
			recall();
		} else {
			eval(recall);
		...
		if(!evaled) evalscript(s);
		ajaxframe.loading = 0;
		jq('append_parent').removeChild(ajaxframe);
	};
	if(!ajaxframe) {
		if (BROWSER.ie) {
		...
		ajaxframe.loading = 1;
		jq('append_parent').appendChild(ajaxframe);
	...
	showloading();
	jq(formid).target = ajaxframeid;
	jq(formid).action += '&inajax=1';
	jq(formid).submit();
	return false;
}
...
function ajaxmenu(ctrlObj, timeout, cache, duration, pos, recall) {
	...
	if(menu) {
		...
		jq('append_parent').appendChild(menu);
...
function showloading(display, waiting) {
	...
	jq('ajaxwaitid').innerHTML = waiting;
	jq('ajaxwaitid').style.display = display;
}
...
function ajaxinnerhtml(showid, s) {
	if(showid.tagName != 'TBODY') {
		...
		jq('append_parent').appendChild(div1);
...
function AC_DetectFlashVer(reqMajorVer, reqMinorVer, reqRevision) {
	...
	if(navigator.plugins != null && navigator.plugins.length > 0 && (navigator.plugins["Shockwave Flash 2.0"] || navigator.plugins["Shockwave Flash"])) {
		...
		try {
			...
			versionStr = axo.GetVariable("jqversion");
		} catch (e) {}
	}
	if(versionStr == -1 ) {
		return false;
...
function AC_FL_RunContent() {
	...
	if(AC_DetectFlashVer(9,0,124)) {
		...
		str = '此内容需要 Adobe Flash Player 9.0.124 或更高版本<br /><a href="http://www.adobe.com/go/getflashplayer/" target="_blank"><img src="http://www.adobe.c../image/shared/download_buttons/get_flash_player.gif" alt="下载 Flash Player" /></a>';
	}
	return str;
}

function simulateSelect(selectId) {
	var selectObj = jq(selectId);
	...
	for(var i = 0; i < selectObj.options.length; i++) {
		...
		li.onclick = function() {
			if(jq(selectId + '_ctrl').innerHTML != this.innerHTML) {
				var lis = menuObj.getElementsByTagName('li');
				lis[jq(selectId).getAttribute('selecti')].className = '';
				this.className = 'current';
				jq(selectId + '_ctrl').innerHTML = this.innerHTML;
				jq(selectId).setAttribute('selecti', this.k_id);
				jq(selectId).options.length = 0;
				jq(selectId).options[0] = new Option('', this.k_value);
	...
	jq(selectId + '_ctrl').onclick = function(e) {
		jq(selectId + '_ctrl_menu').style.width = (BROWSER.ie ? Math.max(jq(selectId + '_ctrl').clientWidth, parseInt(jq(selectId + '_ctrl').currentStyle.width)) : jq(selectId + '_ctrl').clientWidth) + 'px';
		showMenu({'ctrlid':(selectId == 'loginfield' ? 'account' : selectId + '_ctrl'),'menuid':selectId + '_ctrl_menu','evt':'click','pos':'13'});
		doane(e);
	};
	jq(selectId + '_ctrl').onfocus = menuObj.onfocus = function() {
		_attachEvent(document.body, 'keydown', handleKeyDown);
	};
	jq(selectId + '_ctrl').onblur = menuObj.onblur = function() {
		_detachEvent(document.body, 'keydown', handleKeyDown);
	};
	jq(selectId + '_ctrl').onkeyup = function(e) {
		e = e ? e : window.event;
		value = e.keyCode;
		if(value == 40 || value == 38) {
			if(menuObj.style.display == 'none') {
				jq(selectId + '_ctrl').click();
			} else {
...
function switchTab(prefix, current, total) {
	for(i = 1; i <= total;i++) {
		jq(prefix + '_' + i).className = '';
		jq(prefix + '_c_' + i).style.display = 'none';
	}
	jq(prefix + '_' + current).className = 'current';
	jq(prefix + '_c_' + current).style.display = '';
}
...
function showselect(obj, inpid, t, rettype) {
	if(!obj.id) {
		...
		if(!t) {
			...
			if(jq(t)) {
				var lis = jq(t).getElementsByTagName('LI');
				for(i = 0;i < lis.length;i++) {
					s += '<a href="javascript:;" onclick="jq(\'' + inpid + '\').value = this.innerHTML">' + lis[i].innerHTML + '</a><br />';
				}
				s += showselect_row(inpid, '自定义', -1);
			} else {
				s += '<a href="javascript:;" onclick="jq(\'' + inpid + '\').value = \'0\'">永久</a><br />';
...
function showselect_row(inpid, s, v, notime, rettype) {
	if(v >= 0) {
		if(!rettype) {
			...
			d = new Date();
			d.setTime(t);
			return '<a href="javascript:;" onclick="jq(\'' + inpid + '\').value = \'' + d.getFullYear() + '-' + (d.getMonth() + 1) + '-' + d.getDate() + (!notime ? ' ' + d.getHours() + ':' + d.getMinutes() : '') + '\'">' + s + '</a><br />';
		} else {
			return '<a href="javascript:;" onclick="jq(\'' + inpid + '\').value = \'' + v + '\'">' + s + '</a><br />';
		}
	} else if(v == -1) {
		return '<a href="javascript:;" onclick="jq(\'' + inpid + '\').focus()">' + s + '</a><br />';
	} else if(v == -2) {
		return '<a href="javascript:;" onclick="jq(\'' + inpid + '\').onclick()">' + s + '</a><br />';
	}
}
...
function showColorBox(ctrlid, layer, k) {
	if(!jq(ctrlid + '_menu')) {
		...
		for(var i = 0; i < 40; i++) {
			...
			+ (typeof wysiwyg == 'undefined' ? 'seditor_insertunit(\'' + k + '\', \'[color=' + coloroptions[i] + ']\', \'[/color]\')' : (ctrlid == editorid + '_cmd_table_param_4' ? 'jq(\'' + ctrlid + '\').value=\'' + coloroptions[i] + '\';hideMenu(2)' : 'discuzcode(\'forecolor\', \'' + coloroptions[i] + '\')'))
...
function announcement() {
	var ann = new Object();
	ann.anndelay = 3000;ann.annst = 0;ann.annstop = 0;ann.annrowcount = 0;ann.anncount = 0;ann.annlis = jq('annbody').getElementsByTagName("LI");ann.annrows = new Array();
	ann.announcementScroll = function () {
		...
		if(!this.annst) {
			...
			if(this.annrows.length == 1) {
				jq('ann').onmouseover = jq('ann').onmouseout = null;
			} else {
				this.annrows[this.annrowcount] = this.annrows[1];
				jq('annbodylis').innerHTML += jq('annbodylis').innerHTML;
				this.annst = setTimeout(function () { ann.announcementScroll(); }, this.anndelay);
				jq('ann').onmouseover = function () { ann.annstop = 1; };
				jq('ann').onmouseout = function () { ann.annstop = 0; };
			}
			this.annrowcount = 1;
			return;
		}
		if(this.annrowcount >= this.annrows.length) {
			jq('annbody').scrollTop = 0;
			this.annrowcount = 1;
			this.annst = setTimeout(function () { ann.announcementScroll(); }, this.anndelay);
		} else {
			this.anncount = 0;
...
function smilies_show(id, smcols, seditorkey) {
	if(seditorkey && !jq(seditorkey + 'smilies_menu')) {
		var div = document.createElement("div");
		...
		jq(seditorkey + 'smilies_menu').appendChild(div);
	}
...
function smilies_onload(id, smcols, seditorkey) {
	...
	if(typeof smilies_type == 'object') {
		...
		for(i in smilies_type) {
			if(smilies_type[i][0]) {
				smiliestype += '<li><a href="javascript:;" hidefocus="true" ' + (CURRENTSTYPE == i ? 'class="current"' : '') + ' id="'+seditorkey+'stype_'+i+'" onclick="smilies_switch(\'' + id + '\', \'' + smcols + '\', '+i+', 1, \'' + seditorkey + '\');if(CURRENTSTYPE) {jq(\''+seditorkey+'stype_\'+CURRENTSTYPE).className=\'\';}this.className=\'current\';CURRENTSTYPE='+i+';doane(event);">'+smilies_type[i][0]+'</a></li>';
			}
		}
		smiliestype += '</ul></div>';
		jq(id).innerHTML = smiliestype + '<div id="' + id + '_data"></div><div class="smilieslist_page" id="' + id + '_page"></div>';
...
function smilies_switch(id, smcols, type, page, seditorkey) {
	...
	jq(id + '_data').innerHTML = smiliesdata;
	jq(id + '_page').innerHTML = smiliespage;
}

function smilies_preview(seditorkey, id, obj, w) {
	var menu = jq('smilies_preview');
	if(!menu) {
		menu = document.createElement('div');
	...
	mpos = fetchOffset(jq(id + '_data'));
	spos = fetchOffset(obj);
	pos = spos['left'] >= mpos['left'] + jq(id + '_data').offsetWidth / 2 ? '13' : '24';
...
function seditor_insertunit(key, text, textend, moveend) {
	jq(key + 'message').focus();
	textend = isUndefined(textend) ? '' : textend;
	moveend = isUndefined(textend) ? 0 : moveend;
	startlen = strlen(text);
	endlen = strlen(textend);
	if(!isUndefined(jq(key + 'message').selectionStart)) {
		var opn = jq(key + 'message').selectionStart + 0;
		if(textend != '') {
			text = text + jq(key + 'message').value.substring(jq(key + 'message').selectionStart, jq(key + 'message').selectionEnd) + textend;
		}
		jq(key + 'message').value = jq(key + 'message').value.substr(0, jq(key + 'message').selectionStart) + text + jq(key + 'message').value.substr(jq(key + 'message').selectionEnd);
		if(!moveend) {
			jq(key + 'message').selectionStart = opn + strlen(text) - endlen;
			jq(key + 'message').selectionEnd = opn + strlen(text) - endlen;
		}
...
function parseurl(str, mode, parsecode) {
	...
	if(parsecode) str= str.replace(/\s*\[code\]([\s\S]+?)\[\/code\]\s*/ig, function(jq1, jq2) {return codetag(jq2);});
	str = str.replace(/([^>=\]"'\/]|^)((((https?|ftp):\/\/)|www\.)([\w\-]+\.)*[\w\-\u4e00-\u9fa5]+\.([\.a-zA-Z0-9]+|\u4E2D\u56FD|\u7F51\u7EDC|\u516C\u53F8)((\?|\/|:)+[\w\.\/=\?%\-&~`@':+!]*)+\.(jpg|gif|png|bmp))/ig, mode == 'html' ? 'jq1<img src="jq2" border="0">' : 'jq1[img]jq2[/img]');
	str = str.replace(/([^>=\]"'\/@]|^)((((https?|ftp|gopher|news|telnet|rtsp|mms|callto|bctp|ed2k|thunder|synacast):\/\/))([\w\-]+\.)*[:\.@\-\w\u4e00-\u9fa5]+\.([\.a-zA-Z0-9]+|\u4E2D\u56FD|\u7F51\u7EDC|\u516C\u53F8)((\?|\/|:)+[\w\.\/=\?%\-&~`@':+!#]*)*)/ig, mode == 'html' ? 'jq1<a href="jq2" target="_blank">jq2</a>' : 'jq1[url]jq2[/url]');
	str = str.replace(/([^\w>=\]"'\/@]|^)((www\.)([\w\-]+\.)*[:\.@\-\w\u4e00-\u9fa5]+\.([\.a-zA-Z0-9]+|\u4E2D\u56FD|\u7F51\u7EDC|\u516C\u53F8)((\?|\/|:)+[\w\.\/=\?%\-&~`@':+!#]*)*)/ig, mode == 'html' ? 'jq1<a href="jq2" target="_blank">jq2</a>' : 'jq1[url]jq2[/url]');
	str = str.replace(/([^\w->=\]:"'\.\/]|^)(([\-\.\w]+@[\.\-\w]+(\.\w+)+))/ig, mode == 'html' ? 'jq1<a href="mailto:jq2">jq2</a>' : 'jq1[email]jq2[/email]');
	if(parsecode) {
...
function showimmestatus(imme) {
	...
	jq('imme_status_' + imme.id.substr(0, imme.id.indexOf('@'))).innerHTML = lang[imme.statusText];
}
