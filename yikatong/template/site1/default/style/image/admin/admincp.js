...
function showMenu(v) {
	...
	var dragobj = drag && jq("#" + drag)[0] ? jq("#" + drag)[0] : menuObj;
	...
	if (attr == '') {
		...
		var hide = function() {
			if (menuObj.cache) {
				...
				if (menuObj.cover) jq("#" + attr + '_cover')[0].parentNode.removeChild(jq("#" + attr + '_cover')[0]);
			}
			var tmp = [];
...
function reset_style(id) {
	jq("#c" + id + "_v")[0].value="";
	jq('#c'+id+'').css('background', '#000');
	var objsubject = jq('#'+id+'')[0];
	var objfontcolor = jq('#c'+id+'_v')[0];
	jq('#em'+id)[0].checked = false;
	jq('#strong'+id)[0].checked = false;
	jq('#underline'+id)[0].checked = false;
	jq('#c'+id+'').css('background', '#000');
...
function load_style(id, color) {
	var objsubject = jq('#'+id)[0];
	var objfontcolor = jq('#c'+id+'_v')[0];
	var objem = jq('#em'+id)[0];
	var objstrong = jq('#strong'+id)[0];
	var objunderline = jq('#underline'+id)[0];

	// jq.style.color 在 IE 下和firefox下的值不一样

	var colorstr = objsubject.style.color;

	if (jq.trim(color) != "") {
		jq('#c'+id+'_v').val(color);
		jq('#c'+id+'').css('background', color);
	} else {
		jq('#c'+id+'').css('background', '#000');
	}
...
function start_edit_album_subject (album_id) {
	jq("#label_subject_" + album_id).html('');
	jq('#div_subject_' + album_id).show();
	jq('#input_subject_' + album_id).focus();
}

function start_edit_photo_subject(photo_id) {
	jq("#label_subject_" + photo_id).html('');
	jq('#div_subject_' + photo_id).show();
	jq('#input_subject_' + photo_id).focus();
}

function edit_album_subject(album_id, subject) {
	jq.get("?action=ajax&opt=edit_album_subject", {album_id:album_id,subject:subject}, function (data, textStatus){
		this;
		
		jq("#div_subject_" + album_id).hide();
		jq("#label_subject_" + album_id).html(subject + "&nbsp;<img src=\"static/image/ico_edit.png\" />");
	});
}

function edit_photo_subject(photo_id, subject) {
	jq.get("?action=ajax&opt=edit_photo_subject", {photo_id:photo_id,subject:subject}, function (data, textStatus){
		this;
		
		jq("#div_subject_" + photo_id).hide();
		jq("#label_subject_" + photo_id).html(subject + "&nbsp;<img src=\"static/image/ico_edit.png\" />");
	});
}

function delete_item_in_inputtag(display_obj, itemid, input_id) {
	jq(display_obj).parent().fadeOut();

	val = jq("#" + input_id).val();
	val = val.replace(/(^,+)|(,+jq)|(\s*)/g, '');
	val = val.replace(/[,]+/g, ',');
	val = val + ","

	val = val.replace(itemid + ",", '');
	val = val.replace(/(,jq)/g, '');
	jq("#" + input_id).val(val)
