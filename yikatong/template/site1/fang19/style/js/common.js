...
function brand_search(form){
	mod = jq("input[name=mod]:checked").val();
	if ("shop" == mod) {
		form.action = "plugin.php?id=yikatong:brand&bication=street";
	} else if ("consume" == mod) {
		form.action = "plugin.php?id=yikatong:brand&bication=consume";
	} else if ("goods" == mod) {
		form.action = "plugin.php?id=yikatong:brand&bication=goodsearch";
	} else if ("groupbuy" == mod) {
		form.action = "plugin.php?id=yikatong:brand&bication=groupbuy";
	}
}
...
function pm_send(pm_to) {
	url = 'plugin.php?id=yikatong:brand&bication=pm&act=sendbox&msgto=' + pm_to + '&inajax=1';
...
function pm_view(pm_type) {
	url = 'plugin.php?id=yikatong:brand&bication=pm&act=list&msgtype=' + pm_type + '&inajax=1&time=' + (new Date().getTime());
...
function show_pm_box() {
	dialog_with = "520";
	if (!jq("#pm_border")[0]) {
		jq("#append_parent").append('<div id="pm_border" class="dialog" style="display:none; width:' + dialog_with + 'px;"></div>');
	}
	jq("#pm_border").html('<div onclick="pm_close();" style="margin-left:' + (dialog_with - 40) + 'px;" class="btn_close">关闭</div> '
			+ '<iframe id="iframe_pm" scrolling="no" frameborder=0 width="100%" height="100%"></iframe>');
	jq("#pm_border").css({
		"top": (jq(document).scrollTop() + 100) + "px",
		"left": (jq(document).width() - 520) / 2 + "px"
	});
}
...
function showWindow(action, type, id, title) {
	url = 'plugin.php?id=yikatong:brand&bication=report&action=' + action + '&type=' + type + '&id=' + id;
	if (action == "moreshops") {
		show_message('<iframe id="iframe_pm" scrolling="no" frameborder=0 width="520" height="500" src="' + url + '"></iframe>', '店铺管理', 2);
		showMask();
		jq("#" + action + "div").fadeIn("slow");
	} else {
		show_message('', '', 2, url);
		showMask();
		jq("#" + action + "div").fadeIn("slow");
...
function show_message(message, title, type, url, width, height) {
	...
	win_dialog = "<div style=\"position: "+dialog_position+"; z-index: 1001; left: " + (jq(document).width() - 520) / 2 + "px; top: " +
	...
	if (!jq("#append_dialog")[0]) {
		jq("#append_parent").append("<div id=\"append_dialog\"></div>");
	}

	jq("#append_dialog").append(win_dialog);

	if (message.indexOf("alert_") == -1 &&  message.indexOf("iframe") == -1) {
		jq('#fwin_content_' + message_id).append("<div class=\"alert_info\" style=\"margin:10px;\"><p>" + message + "</p></div>");
	} else {
		jq('#fwin_content_' + message_id).append(message);
	}

	if (url) {
		jq('#fwin_content_' + message_id).load(url + "&ajaxid=" + message_id, function(){
			jq("#fwin_dialog_" + message_id).css({
				"top": ((jq(window).height() - jq("#fwin_dialog_" + message_id).height())/2) + "px",
				"left": (jq(window).width() - jq("#fwin_dialog_" + message_id).width()) / 2 + "px"
			});
			if(jq('#fwin_content_' + message_id).html().indexOf("form") == -1) {
				jq("#fwin_pns_" + message_id).show();
			}
			jq("#fwin_dialog_" + message_id).show();
		});
	} else {
		jq("#fwin_dialog_" + message_id).css({
			"top": ((jq(window).height() - jq("#fwin_dialog_" + message_id).height())/2) + "px",
	...
	if (BROWSER.ie && BROWSER.ie < 7) {
		jq("#fwin_content_" + message_id).find("iframe").each(function() {
				this.contentWindow.location.reload(true);
		});
	}

	// 如果内容中出现了表单绑定 AJAX
	jq("#fwin_content_" + message_id).find("form").each(function() {
		bindform(this.id);
	});
...
function submit_content(message_id) {
	formid = jq((jq('#fwin_content_' + message_id).find("form"))[0]).attr("id");
...
function close_message(message_id) {
	jq("#fwin_dialog_" + message_id).remove();
	jq("#append_dialog").remove();
	hideMask();
}
...
function showMask() {
	if (!jq("#mask_div")[0]) {
		...
		jq("#append_parent").append('<div style="height: ' + bh + 'px; display: ;" id="mask_div"></div>');
	}
	jq("#mask_div").css({"display" : '',
		"position" : "absolute",
		"left" : "0",
		"z-index" : "999",
		"margin" : "0",
...
function updateseccode() {
	var img = 'plugin.php?id=yikatong:brand&bication=seccode&rand=' + Math.random();
	if (jq("#img_seccode")) {
		jq("#img_seccode").attr('src', img);
	}
}

function showseccode() {
	if (jq("#login_authcode_img").css('display') != 'block') {
		jq("#login_authcode_img").css('display', 'block');
	}
}

function addseccode() {
	jq("#login_authcode_img").show();
}

function updatecomseccode(height) {
	var img = 'plugin.php?id=yikatong:brand&bication=seccode&rand=' + Math.random() + '&h=' + height;
	if (jq("#img_comseccode")) {
		jq("#img_comseccode").attr('src', img);
	}
}

function showcomseccode() {
	if (jq("#com_authcode_img").css('display') != 'block') {
		jq("#com_authcode_img").css('display', 'block');
	}
}

function addcomseccode() {
	if (jq("#com_authcode_img").css('display') == 'block') {
		jq("#com_authcode_img").css('display', 'none');
	}
}
...
function ajaxform_failed(display_area, data) {
	data.find("item").each(function() {
		obj = jq(this).attr('name');
		msg = jq(this).text();

		input_box = jq('#' + obj);

		if (!input_box[0]) {
			if (jq(":input[name='" + obj + "']")[0]) {
				input_box = jq(":input[name='" + obj + "']");
			}

			if (!input_box[0]) {
				if (jq('#input_' + obj)[0]) {
					input_box = jq('#input_' + obj);
				}
			}
		}
		...
		if (input_box[0]) {
			...
			input_box.bind("focus", function() {
				...
				jq('#span_' + jq(this).attr("name").replace('input_', '')).css('color', '#999999');
				jq('#ajax_status_display').html('');
			});

		}

		if (jq('#span_' + obj)[0]) {
			jq('#span_' + obj).css('color', 'red');
			jq('#span_' + obj).html(msg);
		} else {
			alert(msg);
		}
	});
...
function ajaxform_newcomment(display_area, data) {
	msg = jq.trim(data.find("message").text());
	content = jq.trim(data.find("content").text());
	if (content != '') {
		jq('#postlistreply').append(content);
		jq('#commentmessage').val('');
	}
}

function ajaxform_newrecomment(display_area, data) {
	url = jq.trim(data.find("url").text());
	msg = jq.trim(data.find("message").text());
	upcid = jq.trim(data.find("upcid").text());
	content = jq.trim(data.find("content").text());
	if (content != '') {
		jq('#commentdl' + upcid).append(content);
		jq('#commentmessage').val('');
...
function ajaxform_ok(display_area, data) {
	...
	if (url != "") {
		...
		if (jq("#" + display_area)[0]) {
			jq("#" + display_area).html(msg);
		} else {
			alert(msg);
		}
	}
...
function bindform(formname, display_area) {
	jq(document).ready(function() {
		...
		var ajaxframe;
		var io;
		var formaction = jq('#' + formname)[0].action;
		var __form = jq('#' + formname);
		...
		try {
			...
			__test_frame = jq('<iframe id="__test_' + ajaxframeid + '" name="__test_' + ajaxframeid + '"  width="100" height="100" />');
			...
			data = jq(__test_doc.XMLDocument ? __test_doc.XMLDocument: __test_doc);
			__test_frame.remove();
			
		}catch(err) { __test_frame.remove(); return;}
		
		jq('#' + formname).submit(function() {
			ajaxframe = jq('<iframe id="' + ajaxframeid + '" name="' + ajaxframeid + '"  onload="(jQuery(this).data(\'ajaxform-onload\'))()" />');
			...
			function hanlde_data() {
				...
				if (doc.XMLDocument || jq.isXMLDoc(doc)) {
					data = jq(doc.XMLDocument ? doc.XMLDocument: doc);
					message = data.find("message").text();

					if (message != '') {
						if (!jq('#' + formname + ' #ajax_status_display')[0]) {
							...
							jq('#' + formname + ' #ajax_status_display').html('<span style="color:red;font-weight:bold;">' + message + '</span>');
						}
					}
...
function error_trace(display_area, data) {
	 jq('#' + display_area).html("<div id=\"div_error_trace\" style=\"background:#E1E1E1; padding:5px; width:600px; z-index:10000;\"><div style=\"background:#fff; border:#A7A6A6 1px solid; text-align:left; padding:10px;\">"+data+"</div></div>");
}
...
function createmultiselect(select_id, select_name, select_content, select_parent, select_init_val) {
	var selector = 1;
	jq('#' + select_id).attr('name', select_name);
	jq('#' + select_id).bind('change', function() {
		creat(this.id);
	});
	function creat(id) {
		var originalrid = jq('#' + id + '').val();
		...
		var selectlength = jq('#' + select_parent + ' select').length;
		if (selectlength > 1) {
			for (var i = selectlength; i > 0; i--) {
				var cid = jq('#' + select_parent + ' select:nth-child(' + i + ')').attr('id');
				if (cid.split("_")[1] > csid) {
					jq('#' + select_parent + ' select:nth-child(' + i + ')').remove();
				}
			}

		}
		if (newinnercontent != '') {
			jq("#" + id + "").after('<select><option value="-1">' + select_init_val + '</option>' + newinnercontent + '</select>');
			jq("#" + id + "").removeAttr("name");
			jq("#" + id + " + select").attr('name', select_name);
			jq("#" + id + " + select").attr('id', 'selector' + select_name + '_' + selector);
			jq("#" + id + " + select").bind('change', function() {
				creat(this.id);
			});
		} else {
			jq("#" + id).attr('name', select_name);
		}
		selector = jq('#' + select_parent + ' select').length;
		if (selector == 1) {
			jq('#' + select_id).attr('name', select_name);
		}
	}
...
function show_comment_score_area() {
	show = jq("#comment_score_area").css('display');
	if(show == 'none') {
		jq("#comment_score_area").show();
		jq("#ico_opt").attr('src','static/image/ico_dec.png');
		jq("#ico_opt").attr('title','收起');
	} else {
		jq("#comment_score_area").hide();
		jq("#ico_opt").attr('src','static/image/ico_add.png');
		jq("#ico_opt").attr('title','展开');
	}
}
...
function showMenu(v) {
	...
	var ctrlObj = jq("#" + ctrlid)[0];
	var menuObj = jq("#" + menuid)[0];
	if(!menuObj) return;
	...
	if(!menuObj.initialized) {
		...
		if(cover) {
			...
			jq('#append_parent')[0].appendChild(coverObj);
	...
	jq(menuObj).show();
	if(cover) jq("#" + menuid + '_cover')[0].style.display = '';
	if(fade) {
		var O = 0;
...
function hideMenu(attr, mtype) {
	...
	if(attr == '') {
		...
		var menuObj = jq("#" + attr)[0];
		if(!menuObj || (mtype && menuObj.mtype != mtype)) return;
		clearTimeout(JSMENU['timer'][attr]);
		var hide = function() {
			if(menuObj.cache) {
				menuObj.style.display = 'none';
				if(menuObj.cover) jq("#" + attr + '_cover')[0].style.display = 'none';
			}else {
				menuObj.parentNode.removeChild(menuObj);
				if(menuObj.cover) jq("#" + attr + '_cover')[0].parentNode.removeChild(jq(attr + '_cover'));
			}
			var tmp = [];
...
function setMenuPosition(showid, menuid, pos) {
	...
	var showObj = jq("#" + showid)[0];
	var menuObj = menuid ? jq("#" + menuid)[0] : jq("#" + showid + '_menu')[0];
...
function updateListView(mname, viewmod, viewBy) {
	...
	if (location.href.indexOf("?") == -1 && a[1] == 'html' ) {
		var url = location.href.replace(/([a-z]+?)(-(\d+?)(-(\d+?)*))*\.html/, "plugin.php?id=yikatong:brand&mod=street&catid=$3&tagid=$5&orderby=1");
		location.href = url;
