...
if($subop == 'mokuailist') {
	if(!submitcheck('submit')) {
		...
		foreach($mokuai_array as $k=>$row ){
			showtablerow('', array('style="width:45px"', 'valign="top" style="width:320px"', 'valign="top"', 'align="right" valign="top" style="width:200px"'), array(
				...
				'<span class="bold">'.$row['name'].'-'.$row['version'].'</span>  <span class="sml">('.str_replace("yiqixueba_","",$row['identifier']).')</span>'.$row['identifier'].'<br />'.$row['description'],
				lang('plugin/'.$plugin['identifier'],'price').$row['price'],
				(DB::result_first("SELECT count(*) FROM ".DB::table('yiqixueba_mokuai')." WHERE identifier='".$row['identifier']."'")==0 ? ("<a href=\"".ADMINSCRIPT."?action=".$this_page."&subop=install&mokuaiid=$row[mokuaiid]\" >".lang('plugin/'.$plugin['identifier'],'install')."</a>&nbsp;&nbsp;"):"<a href=\"".ADMINSCRIPT."?action=".$this_page."&subop=update&mokuaiid=$row[mokuaiid]\" >".lang('plugin/'.$plugin['identifier'],'update')."1</a>&nbsp;&nbsp;<a href=\"".ADMINSCRIPT."?action=".$this_page."&subop=uninstall&mokuaiid=$row[mokuaiid]\" >".lang('plugin/'.$plugin['identifier'],'uninstall')."</a>")."<br /><br />".lang('plugin/'.$plugin['identifier'],'status')."<input class=\"checkbox\" type=\"checkbox\" name=\"statusnew[".$row['mokuaiid']."]\" value=\"1\" ".($row['available'] > 0 ? 'checked' : '').">&nbsp;&nbsp;".lang('plugin/'.$plugin['identifier'],'displayorder')."<INPUT type=\"text\" name=\"newdisplayorder[]\" value=\"".$row['displayorder']."\" size=\"2\">",
			));
		}
		showsubmit('submit');
		showtablefooter();
	...
	if(DB::result_first("SELECT count(*) FROM ".DB::table('yiqixueba_mokuai')." WHERE identifier='".$mokuai_array[$mokuaiid]['identifier']."'")==0){
		...
		DB::update('yiqixueba_mokuai', $mokuai_array[$mokuaiid],array('identifier'=>$mokuai_array['identifier']));
	}
