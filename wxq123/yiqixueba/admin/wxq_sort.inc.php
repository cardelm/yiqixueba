...
/**
*	一起学吧平台程序
*	文件名：wxq_sort.inc.php  创建时间：2013-6-14 12:34  杨文
*
*/
...
$this_page = 'plugins&identifier=yiqixueba&pmod=admin&submod=wxq_sort';
...
$sort_info = $sortid ? DB::fetch_first("SELECT * FROM ".DB::table('yiqixueba_wxq123_sort')." WHERE sortid=".$sortid) : array();

$mokuaiid = $sortid ? $sort_info['mokuaiid'] :intval(getgpc('mokuaiid'));

$mokuai_data = array();
$query = DB::query("SELECT * FROM ".DB::table('yiqixueba_mokuai')."  WHERE upmokuai='wxq123' order by displayorder asc");
...
if($subac == 'sortlist') {
	if(!submitcheck('submit')) {
		showtips(lang('plugin/yiqixueba','wxq_sort_list_tips'));
		...
		showtablefooter();
		if($mokuaiid){
			showtableheader(lang('plugin/yiqixueba','wxq_sort_list'));
			...
			$query = DB::query("SELECT * FROM ".DB::table('yiqixueba_wxq123_sort')." WHERE mokuaiid=".$mokuaiid." order by concat(upids,'-',sortid) asc");
			while($row = DB::fetch($query)) {
		...
		if($idg = $_GET['delete']) {
			...
			if($idg) {
				DB::delete('yiqixueba_wxq123_sort', DB::field('sortid', $idg));
			}
		}
		$displayordernew = $_GET['displayordernew'];
		if(is_array($displayordernew)) {
			foreach ( $displayordernew as $k=>$v) {
				DB::update('yiqixueba_wxq123_sort',array('displayorder'=>intval($v)),array('sortid'=>$k));
			}
		}
	...
	if(!submitcheck('submit')) {
		...
		$query = DB::query("SELECT * FROM ".DB::table('yiqixueba_wxq123_sort')." order by concat(upids,'-',sortid) asc");
		while($row = DB::fetch($query)) {
		...
		if($sortid) {
			DB::update('yiqixueba_wxq123_sort',$data,array('sortid'=>$sortid));
		}else{
