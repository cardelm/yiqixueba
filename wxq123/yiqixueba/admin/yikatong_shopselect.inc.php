...
/**
*	一起学吧平台程序
*	文件名：yikatong_shopselect.inc.php  创建时间：2013-6-5 00:14  杨文
*
*/
//商家筛选页面
...
$this_page = 'plugins&identifier=yiqixueba&pmod=admin&submod=yikatong_shopselect';
...
//如果还没有设置则跳转至设置页面
if(!$shop_table||!$yikatong_fields['shopid']||!$yikatong_fields['shopname']) {
...
if($subac == 'shopselect') {
	if(!submitcheck('submit')) {
		...
		$subtitle = array();
		$subtitle[] = '';
		foreach ($yikatong_fields as $k=>$v){
			$subtitle[] = lang('plugin/yiqixueba','field_'.$k);
		}
		...
		$where = "";
		if($shopname) {
			$where .= " and ".$yikatong_fields['shopname']." like '%".$shopname."%'";
		}
		//已筛选sql
		if($shaixuan == 'shaixuaned'){
			...
			$where .= " and ".$yikatong_fields['shopid']." IN (".implode(",",$new_shopid_array).')';
		//未筛选sql
			...
			$where .= " and ".$yikatong_fields['shopid']." NOT IN (".implode(",",$new_shopid_array).')';
		}
		if($where) {
		...
		$query = DB::query("SELECT * FROM ".DB::table($shop_table).$where." order by ".$yikatong_fields['shopid']." asc limit ".$start.", ".$perpage);
		
		while($row = DB::fetch($query)) {
			$new_shop_info = DB::fetch_first("SELECT * FROM ".DB::table('yiqixueba_yikatong_shop')." WHERE oldshopid = ".intval($row[$yikatong_fields['shopid']]));
			$oldshopname = $new_shop_info['shopname'] ? $new_shop_info['shopname'] : $row[$yikatong_fields['shopname']];

			$shop_url = str_replace("{shopid}",intval($row[$yikatong_fields['shopid']]),$yikatong_setting['yiqixueba_yikatong_shop_url']);
			...
			foreach ($yikatong_fields as $k=>$v){
				if($k == 'shopname'){
					...
					$tablerow_old[] = $row[$v] ?'<img src="'.$yikatong_setting['yiqixueba_yikatong_shop_logourl'].$row[$v].'" width=80 height=60 />' : '';
				}else{
					$tablerow_old[] = $row[$v];
				}
			}
			$tablerow_old[] = $new_shop_info['oldshopid'] ? lang('plugin/yiqixueba','pass'): "<input class=\"checkbox\" type=\"checkbox\" name=\"passnew[]\" value=\"".$row[$yikatong_fields['shopid']]."\">";
			$tablerow_old[] = $new_shop_info['oldshopid'] ? "<input class=\"checkbox\" type=\"checkbox\" name=\"nopassnew[]\" value=\"".$row[$yikatong_fields['shopid']]."\">" : lang('plugin/yiqixueba','nopass');
		...
		if(is_array($passnew)) {
			foreach ( $passnew as $k=>$v) {
				if($v && DB::result_first("SELECT count(*) FROM ".DB::table('yiqixueba_yikatong_shop')." WHERE oldshopid=".intval($v))==0) {
					$old_data = DB::fetch_first("SELECT * FROM ".DB::table($shop_table)." WHERE ".$yikatong_fields['shopid']."=".intval($v));
					$insert_data = array();
					foreach ($yikatong_fields as $kk=>$vv){
						if($kk=='shopid'){
					...
					DB::insert('yiqixueba_yikatong_shop',$insert_data);
				}
			}
		}
		//不通过操作
		...
		if(is_array($nopassnew)) {
			foreach ( $nopassnew as $k=>$v) {
				if($v && DB::result_first("SELECT count(*) FROM ".DB::table('yiqixueba_yikatong_shop')." WHERE oldshopid=".intval($v))==1) {
					//DB::update('yiqixueba_yikatong_shop', array('oldshopid'=>0),array('oldshopid'=>intval($v)));
					DB::delete('yiqixueba_yikatong_shop',array('oldshopid'=>intval($v)));
				}
			}
		}
