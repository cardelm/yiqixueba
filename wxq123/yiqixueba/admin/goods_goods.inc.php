...
/**
*	一起学吧平台程序
*	文件名：goods_goods.inc.php  创建时间：2013-7-9 15:14  杨文
*
*/
...
$this_page = 'plugins&identifier=yiqixueba&pmod=admin&submod=goods_goods';
...
$goods_info = $goodsid ? DB::fetch_first("SELECT * FROM ".DB::table('yiqixueba_goods_goods')." WHERE goodsid=".$goodsid) : array();
...
if($subac == 'goodslist') {
	if(!submitcheck('submit')) {
		...
		//$query = DB::query("SELECT * FROM ".DB::table('yiqixueba_goods_goods')." order by goodsid asc");
	...
	if(!submitcheck('submit')) {
		...
		foreach ( $datas as $k=>$v) {
			...
			if(!DB::result_first("describe ".DB::table('yiqixueba_goods_goods')." ".$k)) {
				$sql = "alter table ".DB::table('yiqixueba_goods_goods')." add `".$k."` varchar(255) not Null;";
				runquery($sql);
			}
		}
		if($goodsid) {
			DB::update('yiqixueba_goods_goods',$data,array('goodsid'=>$goodsid));
		}else{
			DB::insert('yiqixueba_goods_goods',$data);
		}
