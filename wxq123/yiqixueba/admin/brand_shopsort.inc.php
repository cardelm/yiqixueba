...
/**
*	一起学吧平台程序
*	文件名：brand_shopsort.inc.php  创建时间：2013-6-8 13:22  杨文
*
*/
...
$this_page = 'plugins&identifier=yiqixueba&pmod=admin&submod=brand_shopsort';
...
$shopsort_info = $shopsortid ? DB::fetch_first("SELECT * FROM ".DB::table('yiqixueba_brand_shopsort')." WHERE shopsortid=".$shopsortid) : array();
...
if($subac == 'shopsortlist') {
	if(!submitcheck('submit')) {
		...
		//$query = DB::query("SELECT * FROM ".DB::table('yiqixueba_brand_shopsort')." where sortupid = ".$sortupid." order by concat(upids,'-',shopsortid) asc");
		...
		$query = DB::query("SELECT * FROM ".DB::table('yiqixueba_brand_shopsort')." where sortupid = ".$sortupid." order by displayorder asc");
		while($row = DB::fetch($query)) {
		...
		if($idg = $_GET['delete']) {
			...
			if($idg) {
				DB::delete('yiqixueba_brand_shopsort', DB::field('shopsortid', $idg));
			}
		}
		$displayordernew = $_GET['displayordernew'];
		if(is_array($displayordernew)) {
			foreach ( $displayordernew as $k=>$v) {
				DB::update('yiqixueba_brand_shopsort',array('displayorder'=>intval($v)),array('shopsortid'=>$k));
			}
		}
	...
	if(!submitcheck('submit')) {
		...
		$data['upids'] = intval($_GET['sortupid']) ? trim(DB::result_first("SELECT upids FROM ".DB::table('yiqixueba_brand_shopsort')." WHERE shopsortid=".intval($_GET['sortupid']))).'-'.intval($_GET['sortupid']) : intval($_GET['sortupid']);


		$data['sortlevel'] = $data['sortupid'] ==0 ? 1 : (intval(DB::result_first("SELECT sortlevel FROM ".DB::table('yiqixueba_brand_shopsort')." WHERE shopsortid=".$data['sortupid']))+1);
		$data['displayorder'] = htmlspecialchars(trim($_GET['displayorder']));

		if($shopsortid) {
			DB::update('yiqixueba_brand_shopsort',$data,array('shopsortid'=>$shopsortid));
		}else{
			DB::insert('yiqixueba_brand_shopsort',$data);
		}
