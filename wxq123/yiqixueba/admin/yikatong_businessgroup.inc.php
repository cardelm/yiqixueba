...
/**
*	一起学吧平台程序
*	文件名：yikatong_businessgroup.inc.php  创建时间：2013-6-4 09:37  杨文
*
*/
...
$this_page = 'plugins&identifier=yiqixueba&pmod=admin&submod=yikatong_businessgroup';
...
$businessgroup_info = $businessgroupid ? DB::fetch_first("SELECT * FROM ".DB::table('yiqixueba_yikatong_businessgroup')." WHERE businessgroupid=".$businessgroupid) : array();
...
if($subac == 'businessgrouplist') {
	if(!submitcheck('submit')) {
		...
		showsubtitle(array('', lang('plugin/yiqixueba','businessgroupname'),lang('plugin/yiqixueba','businessnum'), lang('plugin/yiqixueba','businessgroupquanxian'), lang('plugin/yiqixueba','status'), ''));
		$query = DB::query("SELECT * FROM ".DB::table('yiqixueba_yikatong_businessgroup')." order by businessgroupid asc");
	...
	if(!submitcheck('submit')) {
		...
		$businessgroupid ? showhiddenfields($hiddenfields = array('businessgroupid'=>$businessgroupid)) : '';
		...
		showsetting(lang('plugin/yiqixueba','zhanghaoyue'),'businessgroup_info[zhanghaoyue]',$businessgroup_info['zhanghaoyue'],'text','',0,lang('plugin/yiqixueba','zhanghaoyue_comment'),'','',true);
		showsetting(lang('plugin/yiqixueba','zhanghaojifen'),'businessgroup_info[zhanghaojifen]',$businessgroup_info['zhanghaojifen'],'text','',0,lang('plugin/yiqixueba','zhanghaojifen_comment'),'','',true);
		...
		showsetting(lang('plugin/yiqixueba','cardfeiyong'),'businessgroup_info[cardfeiyong]',$businessgroup_info['cardfeiyong'],'text','',0,lang('plugin/yiqixueba','cardfeiyong_comment'),'','',true);

		showsetting(lang('plugin/yiqixueba','cardpice'),'businessgroup_info[cardpice]',$businessgroup_info['cardpice'],'text','',0,lang('plugin/yiqixueba','cardpice_comment'),'','',true);

		//可以使用的结算消费类型
		$xiaofei = dunserialize($businessgroup_info['xiaofei']);
		$xiaofei_array = array();
		...
		showsetting(lang('plugin/yiqixueba','xiaofeitypeshenhe'),'businessgroup_info[xiaofeitypeshenhe]',$businessgroup_info['xiaofeitypeshenhe'],'radio','',0,lang('plugin/yiqixueba','xiaofeitypeshenhe_comment'),'','',true);
...
EOF;
		...
		if($_POST['delete2'] && $_FILES['contractsample']) {
		...
		foreach ( $datas as $k=>$v) {
			...
			if(!DB::result_first("describe ".DB::table('yiqixueba_yikatong_businessgroup')." ".$k)) {
				$sql = "alter table ".DB::table('yiqixueba_yikatong_businessgroup')." add `".$k."` varchar(255) not Null;";
				runquery($sql);
			}
		}
		if($businessgroupid) {
			DB::update('yiqixueba_yikatong_businessgroup',$data,array('businessgroupid'=>$businessgroupid));
		}else{
			DB::insert('yiqixueba_yikatong_businessgroup',$data);
		}
