...
/**
*	平台主程序-模块管理程序
*	filename:mokuai.inc.php createtime:2013-7-30 10:01  yangwen
*
*/
...
$mokuai_info = $mokuaiid ? DB::fetch_first("SELECT * FROM ".DB::table('yiqixueba_mainpro_mokuai')." WHERE mokuaiid=".$mokuaiid) : array();

if($subac == 'mokuailist') {
	if(!submitcheck('submit')) {
		showtips(lang('plugin/yiqixueba_mainpro','mokuai_list_tips'));
		showformheader($this_page.'&subac=mokuailist');
		showtableheader(lang('plugin/yiqixueba_mainpro','mokuai_list'));
		showsubtitle(array('', lang('plugin/yiqixueba_mainpro','ico'),lang('plugin/yiqixueba_mainpro','mokuainame'), lang('plugin/yiqixueba_mainpro','displayorder'), lang('plugin/yiqixueba_mainpro','status'), ''));
		$query = DB::query("SELECT * FROM ".DB::table('yiqixueba_mainpro_mokuai')." order by displayorder asc");
		while($row = DB::fetch($query)) {
			...
			showtablerow('', array('class="td25"','style="width:45px"', 'class="td23"', 'class="td28"'), array(
				...
				"<a href=\"".ADMINSCRIPT."?action=".$this_page."&subac=mokuaiedit&mokuaiid=$row[mokuaiid]\" class=\"act\">".lang('plugin/yiqixueba_mainpro','edit')."</a>",
			));
		}
	echo '<tr><td></td><td colspan="6"><div><a href="'.ADMINSCRIPT.'?action='.$this_page.'&subac=mokuaiedit" class="addtr">'.lang('plugin/yiqixueba_mainpro','add_mokuai').'</a></div></td></tr>';
	...
	if(!submitcheck('submit')) {
		...
		showtips(lang('plugin/yiqixueba_mainpro','mokuai_edit_tips'));
		showformheader($this_page.'&subac=mokuaiedit','enctype');
		showtableheader(lang('plugin/yiqixueba_mainpro','mokuai_edit'));
		$mokuaiid ? showhiddenfields(array('mokuaiid'=>$mokuaiid)) : '';
		showsetting(lang('plugin/yiqixueba_mainpro','mokuaiico'),'mokuaiico',$mokuai_info['mokuaiico'],'filetext','',0,lang('plugin/yiqixueba_mainpro','mokuaiico_comment').$mokuaiicohtml,'','',true);
		showsetting(lang('plugin/yiqixueba_mainpro','mokuainame'),'mokuai_info[mokuainame]',$mokuai_info['mokuainame'],'text','',0,lang('plugin/yiqixueba_mainpro','mokuainame_comment'),'','',true);
		showsubmit('submit');
		showtablefooter();
		showformfooter();
	}else{
		if(!htmlspecialchars(trim($_GET['mokuai_info']['mokuainame']))) {
			cpmsg(lang('plugin/yiqixueba_mainpro','mokuainame_nonull'));
		}
		...
		foreach ( $datas as $k=>$v) {
			...
			if(!DB::result_first("describe ".DB::table('yiqixueba_mainpro_mokuai')." ".$k)) {
				$sql = "alter table ".DB::table('yiqixueba_mainpro_mokuai')." add `".$k."` varchar(255) not Null;";
				runquery($sql);
			}
		}
		if($mokuaiid) {
			DB::update('yiqixueba_mainpro_mokuai',$data,array('mokuaiid'=>$mokuaiid));
		}else{
			DB::insert('yiqixueba_mainpro_mokuai',$data);
		}
		cpmsg(lang('plugin/yiqixueba_mainpro', 'mokuai_edit_succeed'), 'action='.$this_page.'&subac=mokuailist', 'succeed');
	}
